var albums = {
    1:{
        "title": "2012 - 2017",
        "artist": "Against All Logic",
        "embed": "1uzfGk9vxMXfaZ2avqwxod",
        "track": "Some Kind of Game",
        "link1": 2,
        "link2": 3,
        "link3": 4
    },
    2:{
        "title": "Hard Normal Daddy",
        "artist": "Squarepusher",
        "embed": "1CPBXIsx6Km7ZvDmvM0gYS",
        "track": "Beep Street",
        "link1": 5,
        "link2": 44,
        "link3": 52
    },
    3:{
        "title": "Suzuki",
        "artist": "Tosca",
        "embed": "36ogL93DPRBSrAKjenPLwe",
        "track": "Boss On The Boar",
        "link1": 7,
        "link2": 8,
        "link3": 4
    },
    4:{
        "title": "Space Is Only Noise",
        "artist": "Nicolas Jaar",
        "embed": "0tUJcqDuXHNkaPKLN0lQhT",
        "track": "Specters of the Future",
        "link1": 34,
        "link2": 42,
        "link3": 14
    },
    5:{
        "title": "Compro",
        "artist": "Skee Mask",
        "embed": "3yXIkSJWpudtgF0TZuB16U",
        "track": "Soundboy Ext.",
        "link1": 34,
        "link2": 9,
        "link3": 6
    },
    6:{
        "title": "The Lotus Eaters",
        "artist": "mrkgnao",
        "embed": "5QLwiWguhUc3MTgBsk7LcY",
        "track": "Going",
        "link1": 23,
        "link2": 10,
        "link3": 26
    },
    7:{
        "title": "Play",
        "artist": "Moby",
        "embed": "1xB1tmm50ZhXwrNs89u7Jx",
        "track": "Natural Blues",
        "link1": 26,
        "link2": 12,
        "link3": 20
    },
    8:{
        "title": "The Campfire Headphase",
        "artist": "Boards Of Canada",
        "embed": "0wBiN0fKhy0ywx732SpgUs",
        "track": "Dayvan Cowboy",
        "link1": 14,
        "link2": 28,
        "link3": 5
    },
    9:{
        "title": "Twoism",
        "artist": "Board of Canada",
        "embed": "0LDCQOBp7NFjkD1LbuusCG",
        "track": "Sixtyniner",
        "link1": 15,
        "link2": 53,
        "link3": 14
    },
    10:{
        "title": "Draining Love Story",
        "artist": "Sewerslvt",
        "embed": "3yQqw6ZJDHey6HZCpsrFTb",
        "track": "Swinging in His Cell",
        "link1": 16,
        "link2": 35,
        "link3": 27
    },
    11:{
        "title": "The Disintegration Loops",
        "artist": "William Basinski",
        "embed": "1xVP4eFcagXNtQMvEXfyeV",
        "track": "dlp 1.1",
        "link1": 0,
        "link2": 0,
        "link3": 0
    },
    12:{
        "title": "99.9%",
        "artist": "KAYTRANADA",
        "embed": "6JD4Qerb8IcaAzFgpFw0sa",
        "track": "BULLETS",
        "link1": 27,
        "link2": 0,
        "link3": 0
    },
    13:{
        "title": "Tomorrow's Modern Boxes",
        "artist": "Thom Yorke",
        "embed": "48mLH4Pm2BErDEQC7YT1xW",
        "track": "The Mother Lode",
        "link1": 21,
        "link2": 23,
        "link3": 28
    },
    14:{
        "title": "Ambient 1/Music For Airports",
        "artist": "Brian Eno",
        "embed": "063f8Ej8rLVTz9KkjQKEMa",
        "track": "1/1",
        "link1": 11,
        "link2": 42,
        "link3": 15
    },
    15:{
        "title": "Geogaddi",
        "artist": "Boards of Canada",
        "embed": "7Lou3RDk6NswmbzBMhro24",
        "track": "Dawn Chorus",
        "link1": 30,
        "link2": 38,
        "link3": 33
    },
    16:{
        "title": "Drukqs",
        "artist": "Aphex Twin",
        "embed": "48mLH4Pm2BErDEQC7YT1xW",
        "track": "Vordhosbn",
        "link1": 24,
        "link2": 5,
        "link3": 9
    },
    17:{
        "title": "Ceres & Calypso in the Deep Time",
        "artist": "Candy Claws",
        "embed": "5KkGAZrad4N4rc5FriwgzU",
        "track": "Vordhosbn",
        "link1": 25,
        "link2": 5,
        "link3": 54
    },
    18:{
        "title": "Merriweather Post Pavilion",
        "artist": "Animal Collective",
        "embed": "5O9OXl9zAWMJTzawofxuan",
        "track": "My Girls",
        "link1": 0,
        "link2": 0,
        "link3": 0
    },
    19:{
        "title": "Lonerism",
        "artist": "Tame Impala",
        "embed": "3C2MFZ2iHotUQOSBzdSvM7",
        "track": "Feels Like We Only Go Backwards",
        "link1": 18,
        "link2": 0,
        "link3": 0
    },
    20:{
        "title": "Since I Left You",
        "artist": "The Avalanches",
        "embed": "3GBnNRYsxBfEeMSMmTpJ25",
        "track": "Radio",
        "link1": 1,
        "link2": 19,
        "link3": 0
    },
    21:{
        "title": "Cosmogramma",
        "artist": "Flying Lotus",
        "embed": "5c7XChrHxYaqykCZLaGM5f",
        "track": "Computer Face//Pure Being",
        "link1": 30,
        "link2": 24,
        "link3": 10
    },
    22:{
        "title": "Kid A",
        "artist": "Radiohead",
        "embed": "6GjwtEZcfenmOf6l18N7T7",
        "track": "Everything in its Right Place",
        "link1": 0,
        "link2": 0,
        "link3": 0
    },
    23:{
        "title": "Body Riddle",
        "artist": "Clark",
        "embed": "04onGeB7bKwWHPPTXAiszV",
        "track": "Night Knuckles",
        "link1": 30,
        "link2": 21,
        "link3": 13
    },
    24:{
        "title": "Velocity: Design: Comfort",
        "artist": "Sweet Trip",
        "embed": "0eUUQ4rly8Q8PyJPWLgde2",
        "track": "Chocolate Matter",
        "link1": 35,
        "link2": 25,
        "link3": 17
    },
    25:{
        "title": "Loveless",
        "artist": "My Bloody Valentine",
        "embed": "3GH4IiI6jQAIvnHVdb5FB6",
        "track": "I Only Said",
        "link1": 0,
        "link2": 0,
        "link3": 0
    },
    26:{
        "title": "Moon Safari",
        "artist": "Air",
        "embed": "206GTDefY2qRMQxYXmfb0a",
        "track": "La femme d'argent",
        "link1": 32,
        "link2": 31,
        "link3": 43
    },
    27:{
        "title": "Discovery",
        "artist": "Daft Punk",
        "embed": "2noRn2Aes5aoNVsU6iWThc",
        "track": "High Life",
        "link1": 29,
        "link2": 1,
        "link3": 32
    },
    28:{
        "title": "Bicep",
        "artist": "Bicep",
        "embed": "4psDRFbIlUM1KUb1omccXo",
        "track": "Glue",
        "link1": 39,
        "link2": 15,
        "link3": 29
    },
    29:{
        "title": "Ultima II Massage",
        "artist": "TOBACCO",
        "embed": "2G172t9joEhgY1QNYP4nAU",
        "track": "Father Sister Berzerker",
        "link1": 40,
        "link2": 47,
        "link3": 41
    },
    30:{
        "title": "Selected Ambient Works 85-92",
        "artist": "Aphex Twin",
        "embed": "7aNclGRxTysfh6z0d8671k",
        "track": "Actium",
        "link1": 48,
        "link2": 28,
        "link3": 0
    },
    31:{
        "title": "Big Calm",
        "artist": "Morcheeba",
        "embed": "7rjjFxMFZjPzQzBMj6mXr2",
        "track": "The Sea",
        "link1": 38,
        "link2": 0,
        "link3": 0
    },
    32:{
        "title": "Mother Earth's Plantasia",
        "artist": "Mort Garson",
        "embed": "4Pj4zPHuEUPtfacdeTlqUG",
        "track": "Symphony for a Spider Plant",
        "link1": 39,
        "link2": 33,
        "link3": 44
    },
    33:{
        "title": "Uncut Gems - Original Motion Picture Soundtrack",
        "artist": "Daniel Lopatin",
        "embed": "3pvGQtSsi9LGcPIZ7plWYe",
        "track": "The Bet Hits",
        "link1": 44,
        "link2": 46,
        "link3": 0
    },
    34:{
        "title": "In Colour",
        "artist": "Jamie xx",
        "embed": "04Duapg2mNlVykd895xcfZ",
        "track": "Obvs",
        "link1": 35,
        "link2": 47,
        "link3": 33
    },
    35:{
        "title": "Richard D. James Album",
        "artist": "Aphex Twin",
        "embed": "43s2fKRQsOSB6rSrxtAXKK",
        "track": "4",
        "link1": 0,
        "link2": 0,
        "link3": 0
    },
    36:{
        "title": "Malibu Ken",
        "artist": "Malibu Ken",
        "embed": "3Xh4OCCACL8BxKDOl413DI",
        "track": "1+1=13",
        "link1": 0,
        "link2": 0,
        "link3": 0
    },
    37:{
        "title": "Mezzanine",
        "artist": "Massive Attack",
        "embed": "49MNmJhZQewjt06rpwp6QR",
        "track": "Teardrop",
        "link1": 0,
        "link2": 0,
        "link3": 0
    },
    38:{
        "title": "Dummy",
        "artist": "Portishead",
        "embed": "3539EbNgIdEDGBKkUf4wno",
        "track": "Sour Times",
        "link1": 37,
        "link2": 0,
        "link3": 0
    },
    39:{
        "title": "Odyssey",
        "artist": "Home",
        "embed": "2Nz9gdj35Unk1AbfL8Igmx",
        "track": "On the Way Out",
        "link1": 40,
        "link2": 29,
        "link3": 34
    },
    40:{
        "title": "One Hundred Mornings",
        "artist": "Windows 96",
        "embed": "3ggIpl7qJQlrtSQkP5GY1Q",
        "track": "Caligula",
        "link1": 41,
        "link2": 26,
        "link3": 42
    },
    41:{
        "title": "Slide",
        "artist": "George Clanton",
        "embed": "12Biup1Sq5hpHsX85gJPb2",
        "track": "Dumb",
        "link1": 48,
        "link2": 54,
        "link3": 17
    },
    42:{
        "title": "新しい日の誕生",
        "artist": "２８１４",
        "embed": "01m2bictEiKiVzFjk0h28N",
        "track": "恢复",
        "link1": 0,
        "link2": 0,
        "link3": 0
    },
    43:{
        "title": "Emperor Tomato Ketchup",
        "artist": "Stereolab",
        "embed": "4420hj4RNgpozukzJlePCG",
        "track": "The Noise Of Carpet",
        "link1": 0,
        "link2": 0,
        "link3": 0
    },
    44:{
        "title": "The Man Machine",
        "artist": "Kraftwerk",
        "embed": "3eyz60xEK5dGEeZF1JJSi9",
        "track": "The Model",
        "link1": 49,
        "link2": 0,
        "link3": 0
    },
    45:{
        "title": "Interstellar (Original Motion Picture Soundtrack)",
        "artist": "Hans Zimmer",
        "embed": "3B61kSKTxlY36cYgzvf3cP",
        "track": "Cornfield Chase",
        "link1": 52,
        "link2": 51,
        "link3": 39
    },
    46:{
        "title": "TRON: Legacy",
        "artist": "Daft Punk",
        "embed": "4US3nmuLIKELhVZdBPiKxx",
        "track": "Disc Wars",
        "link1": 45,
        "link2": 53,
        "link3": 39
    },
    47:{
        "title": "2017 - 2019",
        "artist": "Nicolas Jaar",
        "embed": "75vFTgDgdfVhbniLoyCNKh",
        "track": "Penny",
        "link1": 48,
        "link2": 35,
        "link3": 45
    },
    48:{
        "title": "Immunity",
        "artist": "Jon Hopkins",
        "embed": "1rxWlYQcH945S3jpIMYR35",
        "track": "Open Eye Signal",
        "link1": 52,
        "link2": 53,
        "link3": 34
    },
    49:{
        "title": "Little Dark Age",
        "artist": "MGMT",
        "embed": "7GjVWG39IOj4viyWplJV4H",
        "track": "Me and Michael",
        "link1": 0,
        "link2": 0,
        "link3": 0
    },
    50:{
        "title": "Dots And Loops",
        "artist": "Stereolab",
        "embed": "5YZmrMsFY6xdUBsS7DJxaz",
        "track": "Ticker-tape of the Unconcious",
        "link1": 0,
        "link2": 0,
        "link3": 0
    },
    51:{
        "title": "Dead Magic",
        "artist": "Anna von Hausswolff",
        "embed": "29haLrvX37jDkDfwVk4FKV",
        "track": "Källans återuppståndelse",
        "link1": 22,
        "link2": 0,
        "link3": 0
    },
    52:{
        "title": "Flamagra",
        "artist": "Flying Lotus",
        "embed": "5WfDyog8yO7ZF8JdJxeZfl",
        "track": "Takashi",
        "link1": 0,
        "link2": 0,
        "link3": 0
    },
    53:{
        "title": "Amnesiac",
        "artist": "Radiohead",
        "embed": "1HrMmB5useeZ0F5lHrMvl0",
        "track": "Knives Out",
        "link1": 22,
        "link2": 0,
        "link3": 0
    },
    54:{
        "title": "Depression Cherry",
        "artist": "Beach House",
        "embed": "194CqC2Zi0kUFEPWedb3qr",
        "track": "PPP",
        "link1": 0,
        "link2": 0,
        "link3": 0
    }
};

var album;


function populate() {
    let args = window.location.search;
    console.log(args);
    if(args === "") {
        location.href = "index.html?album=1"
    }
    let albumId = (parseInt(args.split("=")[1]));
    album = albums[albumId];
    let albumStr = "<iframe src=\"https://open.spotify.com/embed/album/" + album["embed"] + "\" frameborder=\"0\" allowtransparency=\"true\" allow=\"encrypted-media\"></iframe>";
    if(albumId !== 0) {
        document.getElementById("album-iframe").innerHTML = albumStr;
        document.getElementById("album-btn-1").innerHTML = album["link1"] === 0 ? "No Album Here" : (albums[album["link1"]])["title"] + ", " + (albums[album["link1"]])["artist"];
        document.getElementById("album-btn-2").innerHTML = album["link1"] === 0 ? "No Album Here" : (albums[album["link2"]])["title"] + ", " + (albums[album["link2"]])["artist"];
        document.getElementById("album-btn-3").innerHTML = album["link1"] === 0 ? "No Album Here" : (albums[album["link3"]])["title"] + ", " + (albums[album["link3"]])["artist"];
    }

}

function route(option) {
    switch (option) {
        case 1:
            location.href = ("index.html?album=" + album["link1"]);
            break;
        case 2:
            location.href = ("index.html?album=" + album["link2"]);
            break;
        case 3:
            location.href = ("index.html?album=" + album["link3"]);
            break;
        case 4:
            location.href = ("index.html?album=" + Math.floor(Math.random() * 54));
    }
}

window.addEventListener('load', function () {
    populate()
});
